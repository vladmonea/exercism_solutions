...
from triangle import is_equilateral, is_isosceles, is_scalene, is_degenerate
...
class is_degenerateTests(unittest.TestCase):
    def tests_is_degenerate(self):
        self.assertIs(is_degenerate([1, 2, 3]), True)

    def test_equilateral_is_not_degenerate(self):
        self.assertIs(is_degenerate([2, 2, 2]), False)

    def test_isosceles_is_not_degenerate(self):
        self.assertIs(is_degenerate([3, 4, 4]), False)

    def test_scalene_is_not_degenerate(self):
        self.assertIs(is_degenerate([5, 4, 6]), False)
